 ...
 * seven knots, each owning a band of the track proportional to how far back it
...
export function BitcoinTimeline({
  ...
  const band = bandAt(BANDS, position);
  const sample =
    ...
    >
      ...
      {samples === null ? (
        ...
        <dl className="mt-4 grid grid-cols-3 gap-4">
          ...
          <Readout
            label="difficulty"
            value={sample ? formatCompact(sample.difficulty) : "—"}
            unit={
              sample
                ? `${formatCompact(hashrateEhs(sample.difficulty))} EH/s`
                : ""
            }
            title={title}
          />
          <Readout
            label="fee rate"
            value={sample ? sample.feeRateSatVb.toFixed(1) : "—"}
            unit="sat/vB"
            title={title}
          />
        </dl>
