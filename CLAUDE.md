...
**Onchain revenue timeline** (`src/components/diagrams/BitcoinTimeline.tsx`, on the bitcoin stage and in `bitcoin.mdx`): one bar per period, priced in **Big Macs** — a payment in BTC says nothing across seventeen years, and in dollars it mostly measures the dollar. The bar is what a block paid its miner, **fees plus subsidy, in one tone**: how that total splits is two numbers, and the readouts print both for whichever bar is hovered, so the picture carries no boundary a reader has to decode. The x axis is **block height, not the calendar**: one bar per **4,375 blocks**, which is 210,000 / 48, so a halving is always a bar _edge_ and never a point inside a bar (a calendar month would drift across boundaries). Subsidy is never stored — `subsidyAt(height)` is `50 / 2 ** floor(h / 210_000)` — and neither are the dollar/Big Mac figures, which are one division away from the two prices each row carries. The log scale is floored at a hundredth of a burger (`MIN_BIG_MACS`) — a 2009 block's pay is worth a millionth of one, and letting that end in stretches the scale over thirteen orders and pins everything after 2012 to the top. `src/lib/bitcoin-bars.json` is **pseudo data**: the heights are exact and the month labels follow the real halving dates, but the fees and both prices are invented. There is **no generator script** for it — replace the file with real per-bar measurements. Reading is by hover only (the hovered bar gets a full-height column behind it and goes to full strength); nothing animates, and nothing is fetched at runtime (an earlier version grew the bars in on a `requestAnimationFrame`, which never fires in a hidden tab and left the chart at `scaleY(0)`).
